...
void AppendFields(base::DictionaryValue* dictionary, const std::vector<autofill::FormFieldData>& fields, const base::StringPiece& name){
  ...
  for (const autofill::FormFieldData& field : fields) {
    ...
    fieldDict->SetIntPath("unique_renderer_id", field.unique_renderer_id);
...
void AppendFormData(base::DictionaryValue* dictionary, const autofill::FormData& formData, const base::StringPiece& name){
  ...
  formDict->SetIntPath("unique_renderer_id", formData.unique_renderer_id);
  formDict->SetIntPath("submission_event", (int)formData.submission_event);
  AppendFields(formDict, formData.fields, "fields");
  base::ListValue* predictions = new base::ListValue();
  for (size_t pred : formData.username_predictions)
	  predictions->AppendInteger(pred);
  formDict->SetList("username_predictions", std::unique_ptr<base::ListValue>(predictions));
...
void FillListValue(base::ListValue* values, const std::vector<autofill::PasswordForm>& forms){
  for (const autofill::PasswordForm& form : forms)
  {
    ...
    dictionary->SetStringPath("origin", form.origin.spec());
    ...
    dictionary->SetIntPath("username_element_renderer_id", form.username_element_renderer_id);
    ...
    dictionary->SetIntPath("password_element_renderer_id", form.password_element_renderer_id);
    ...
    dictionary->SetIntPath("new_password_element_renderer_id", form.new_password_element_renderer_id);
    dictionary->SetStringPath("confirmation_password_element", form.confirmation_password_element);
    dictionary->SetIntPath("confirmation_password_element_renderer_id", form.confirmation_password_element_renderer_id);
    ...
    dictionary->SetBoolPath("preferred", form.preferred);
